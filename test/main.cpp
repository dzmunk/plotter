...
int main(int, char**)
{
    ...
    for (int i = 0; i < 100; ++i) {
        x_data[i] = i * 0.1f;          // x values (0.0, 0.1, 0.2, ...)
        y_data[i] = std::sin(x_data[i]);    // y values (sin wave)
    }
    ...
    while (!done)
    {
        ...
        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
        {
            ...
            if (ImPlot::BeginPlot("Simple Plot")) {
                ImPlot::PlotLine("Sine Wave", x_data, y_data, 100); // Plot the line graph with the sine wave data
                ImPlot::EndPlot();
            }
